...
class RallyLineGraphChart @JvmOverloads constructor(
    ...
    private fun initAttributes(attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.RallyLineGraphChart)
...
//  }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        viewCanvas = bitmap?.let { Canvas(it) }
...
//        val maxData = data.maxByOrNull { it.amount }!!.amount
        ...
        for (i in 0 until data.size) {
            val y = bottomY - (data[i].amount / (maxData?.times((bottomY - curveTopMargin))!!))
